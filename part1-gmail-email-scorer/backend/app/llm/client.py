...
- Always compare the From display-name to the sender's email address domain. If the display-name claims a brand, service, organization, or role (DHL, Microsoft, DocuSign, your bank, HR, IT support, a coworker's name, "your CEO", a service like Roundcube / Gmail / Outlook, the recipient's own domain, etc.) AND the sender's email address sits on a domain that has no obvious relation to that claimed identity — no shared significant word, no clear vendor or parent-company relationship — set impersonation_target to the claimed identity. This applies to every brand or identity, not just famous ones: if the display-name says "Acme Corp" and the email is from `noreply@randomhost.tk`, that is impersonation even if you have never heard of Acme. The mismatch alone is the signal; you do not need to know whether the brand exists.
- Routine, plausibly-expected correspondence should still produce neutral observations: newsletters the recipient likely subscribed to, replies in an existing thread, system notifications from services they use, personal emails where the display-name is a person's name and the sender domain is a generic provider like gmail.com / outlook.com / yahoo.com (those are not impersonation). The distinction is whether the email is something the recipient was plausibly expecting, vs. an out-of-the-blue email that wants action from them — the latter deserves close observation.
